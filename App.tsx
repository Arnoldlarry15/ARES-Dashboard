...
export default function App() {
  ...
  const [aiSettings, setAiSettings] = useState<AiModelSettings>(() => StorageManager.loadAiSettings());
  ...
  useEffect(() => {
    StorageManager.saveAiSettings(aiSettings);
  }, [aiSettings]);
  ...
  }, [selectedTactic, currentStep, selectedVectors, selectedPayloadIndices, showCampaignModal, showSaveCampaignModal, showKeyboardShortcuts, showAiSettings]);
  ...
  const handleTacticSelect = async (tactic: TacticMetadata) => {
    ...
    try {
      const details = await gemini.generateTacticDetails(tactic, aiSettings);
  ...
  const handleSaveAiSettings = () => {
    ...
    setNotification('AI settings saved');
    setTimeout(() => setNotification(null), 2000);
  };
  ...
  const handleResetAiSettings = () => {
    ...
    StorageManager.saveAiSettings(defaults);
  };
    ...
    <div className={`min-h-screen flex flex-col relative overflow-hidden ${
      ...
      {theme === 'dark' && (
        <div className="fixed inset-0 z-0">
          ...
          <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-red-600/10 rounded-full blur-3xl animate-pulse pulse-delay-1"></div>
          <div className="absolute top-1/2 right-1/3 w-64 h-64 bg-red-500/5 rounded-full blur-3xl animate-pulse pulse-delay-2"></div>
        </div>
      )}
        ...
        <header className={`glass border-b sticky top-0 z-50 backdrop-blur-xl ${
          ...
          <div className="max-w-7xl mx-auto px-6 h-28 flex items-center justify-between">
            ...
            <div className={`flex items-center gap-3 text-[10px] mono ${theme === 'light' ? 'text-slate-600' : 'text-slate-400'}`}>
             ...
             >
               <Settings2 className="w-3.5 h-3.5 relative z-10" />
               <span className="hidden sm:inline font-bold relative z-10">AI SETTINGS</span>
             </button>
             <button 
              ...
              <div className={`flex-1 border rounded-2xl flex flex-col overflow-hidden shadow-2xl relative ${
                ...
                {currentStep === 'payloads' && (
                  <div className="flex-1 flex flex-col p-6 animate-in fade-in slide-in-from-right-2">
                    <div className="flex items-center justify-between mb-6">
                      <div className="flex items-center gap-3">
                        <button onClick={() => setCurrentStep('vectors')} title="Back to attack vectors" aria-label="Back to attack vectors" className={`p-2 rounded-lg transition-all ${
                              ...
                              isSelected 
                                 ...
                                 theme === 'light' ? 'border-slate-200 bg-slate-50/50' : 'border-slate-800 bg-slate-900/40'
                                  ...
                                  <div className="flex items-center gap-3">
                                    <button 
                                      ...
                                      title={isSelected ? 'Deselect payload' : 'Select payload'}
                                      aria-label={isSelected ? 'Deselect payload' : 'Select payload'}
                               ...
                               <div className="relative group p-5">
                                  ...
                                  <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                    ...
                                    <button onClick={() => copyToClipboard(p.payload)} title="Copy payload" aria-label="Copy payload" className={`p-2 rounded-lg border transition-all ${
                                      theme === 'light'
                ...
                {currentStep === 'export' && (
                  <div className="flex-1 flex flex-col p-6 animate-in fade-in slide-in-from-right-2">
                    <div className="flex items-center gap-3 mb-8">
                       <button onClick={() => setCurrentStep('payloads')} title="Back to payloads" aria-label="Back to payloads" className={`p-2 rounded-lg transition-all ${
                         theme === 'light' ? 'hover:bg-slate-100 text-slate-600' : 'hover:bg-slate-800 text-slate-400'
                       }`}>
                           <ArrowLeft className="w-5 h-5" />
                        </button>
          ...
          <div className={`border rounded-2xl p-6 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200 ${
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <button 
                ...
                title="Close save campaign dialog"
                aria-label="Close save campaign dialog"
                className={`p-1 rounded-lg transition-all ${
                  theme === 'light' ? 'hover:bg-slate-100 text-slate-600 hover:text-slate-900' : 'hover:bg-slate-800 text-slate-400 hover:text-slate-200'
                }`}
              >
          ...
          <div className={`border rounded-2xl p-6 w-full max-w-2xl shadow-2xl animate-in zoom-in-95 duration-200 max-h-[80vh] flex flex-col ${
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <button 
                ...
                title="Close saved campaigns dialog"
                aria-label="Close saved campaigns dialog"
                className={`p-1 rounded-lg transition-all ${
                  theme === 'light' ? 'hover:bg-slate-100 text-slate-600 hover:text-slate-900' : 'hover:bg-slate-800 text-slate-400 hover:text-slate-200'
                }`}
              >
